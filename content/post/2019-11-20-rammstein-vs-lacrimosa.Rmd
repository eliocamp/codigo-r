...
Some time ago, someonw I follow on twitter posted about having to buy a whole book with rules to tease out gramatical gender in German. Further down the replies, someone reminisced about trying (and failing) to learn German just by listening to Rammstein's lyrics. I studied about drei Jahre of German at the same time I started listening to Rammstein and other German-speaking bands and I've always found Rammstein's lyrics to be surprisingly simple. So simple, in fact, that I suspect that Till Lindemann intentionally dumbs down his writting to cater to the English speaking word and, in particular, to beginner-level German learners. 

I decided I wanted to see if this vage idea had any merit. For that I downloaded Rammstein's lyrics and measured it's complexity. As a benchmark, I compared them with Lacrimosa's lyrics, another German-speaking band from my youth. 
...
Now, the song does have some complex meaning and interesting uses of metaphore, but the sentence structure is painfully simple. Not only that, but since it repeats the numbers one through ten over and over again, I cannot shake the feeling that is a song sang to children so they learn the numbers. 
...
[^genius]: There is also the [geniusr](https://github.com/ewenme/geniusr) package which makes use of the official genius API and therefore should be preffered for more intensitve use. However, I discovered after I've already written most of this code. 
...
Related to this, there's the small issue that not all songs by these bands are in German. Most albums by Lacrimosa, for example, have at least one song in English (often voiced by Anne Nurmi instead of Tilo Wolff). So I'll need to detect and remove them from my analysis. Again, a bit of googling lead me to the [textcat](https://cran.r-project.org/web/packages/textcat/index.html) package, which I will consider a black box algorithm because at this point I'm done going through rabbit holes. 
...
# Get the language
full_lyrics[, language := textcat::textcat(lyric), by = .(track_title, artist, album)]
...
As expected, most songs by either band are in German and a non neglible part of Lacrimosa's are in English. The algorithm seems to be fooled by some lyrics, though. I inspected the individual cases. All songs marked as "scots" are actually in english. The songs allegedly in Catalan or Romanian are actually a instrumental songs which {genius} seems to have messed up. The one in "Portuguese" is actually Rammstein's "Te quiero, Puta" which is actually in Spanish. Finally, Lacrimosa's Vankina is actually in Finnish. 

Note that since song lyrics are in verse and don't have punctuation I had to make the bold choice of assuming that each verse is it's onw sentence. 
...
So...hypothesis confirmed? Rammstein's lyrics are definetly on the simple side. That said, they do own the 3 most complex songs in the dataset. However, this might be a bit misleading. Let's look at the most complex song.
...
The simplest song of all is, no surprise, Du hast. It's a rather 'dumb' song, at least with respect of readability. It's got very short verses, very short words and very simple structure. What the SMOG score doesn't capture, though, is that it's also very repetitive, with only 3 distinct stanzas!

This observation again resonates with my experience with both bands. Rammstein adheres fully to the traditional song structure of verses, choruses and bridges. Lacrimosa, on the other hand, is no stranger to more "prose-like" lyrics with little in the way of repetition. For example, [Der Strasse der Zeit](https://www.youtube.com/watch?v=qdzI88X4YZk) has no chorus at all and, thus, ounting the proportion of unique verses in that song we see that only about 7% of the lines are duplicated. 
...
Woah! The difference here is oustanding! Unlike Rammstein, who are happy to write lyrics with 30% of repetition or higher, Lacrimosa write songs that repeat no more than 20% of its verses. A sizeable chunk of their songs feature no repetition at all!
