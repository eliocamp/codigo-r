...
This is not just a Magritte-style observation. The important distinction comes into play when you realise that the monitor you are looking at has just three LEDs that emit light in just three wavelengths (sort of). How can it still reproduce a full spectrum of light? It doesn't. For each (aproximatelly) monocromatic colour in that rainbow, your monitor is actually emitting an unique mixture of red, green and blue light that tricks your visual system (and mine) into seeing the colour associated with that wavelength. 

How that works is unreasonable complex and beyond what I can explain in this article (I do recommend this [amazing article](http://jamie-wong.com/post/color/), though) but the core insight is that our eyes have only three colour receptors that are sensible to wide range of short (S), medium (M) and long (L) wavelengths. Any spectrum distribution that reaches our eyes is reduced to just three numbers: the excitation of the S, M and L receptors. Any spectrum distribution that excites them in the same way will be percived as the same color, even if they are wildly different. In colorimetry this is known as *metamerism* and is the basis for color reproduction in computer screens, printing and painting [@Hunt2004-7]. The monocromatic yellow you see in the picture of the prism looks the same as the red, green and blue mixture of your monitor even though their spectrum distribution is not even remotely similar. They are metamers. 
...
The concept of "data with different graphs but same statistics" is still relevant, with multiple published papers describing methods for their creation [e.g. @Chatterjee2007; @Govindaraju2008; @Haslett2009; @Matejka2017]. In this post I will use the term "metamers" to refer to sets of datasets that have the same behaviour under some statistical transformation. The [metamer](https://github.com/eliocamp/metamer) package implements @Matejka2017 algorithm to construct metamers. The main function, `metamerize()`, generates metamers from an initial dataset and the statistical transformation that needs to be preserved. Optinally, it can take a function that will be minimised in each sucessive metamer. 
...
metamers %>% 
   ...
   knitr::kable(col.names = c(summ_names),
                caption = "Statistical properties of the generated metamers (rounded to two significant figures).")
```
...
```{r tweet, fig.cap = "Download the datasaurus", fig.align="center", echo = FALSE}
knitr::include_graphics("/images/datasaurus_tweet.png")
```

However, I believe there is a more fundamental principle at play. The problem with *summary statistics* is the *summary* part. In many cases, the role of statistics is to sum up data. To take a big set of observations that cannot be grasped in their entiretly because the limitations of our comprehension, and condense them into a few numbers or propertied that we can easily get. The problem is that what is gained in understanding is lost in information. 
...
With each subsequent moment one can get a richer picture of the underlying data. The limit is when one has the same amount of moments as the sample size. A single univariate sample of size N can be univocally described by its N first moments. This makes sense intuitively --why should you need more than N numbers to describe N numbers?-- but it can be [demonstrated](https://math.stackexchange.com/questions/3033407/is-a-sample-of-size-n-uniquely-described-by-n-sample-moments)[^unicidad]. 


[^unicidad]: Technically, the solution is unique up to any permutation. This is not an accident. If the order matters, then it is a case of bivariate samples (each "datum" is actualy a pair of values (x; y)). Intuition tells that besides the moment of each variable, the joint moments (covariance and such) are needed. So it seems plausible that in this case the matrix $A^{N\times N}$, where the element in the ith row and jth column is $x^iy^j$ would be needed; which implies the need of $N^2 -1$ moments. 

In other words, the transformation "first N moments" has no metamers for samples smaller than N, except any permutation of the same sample (but see [1](#fn1)).

But this property is not exclusive to statistical moments. The sames goes for the fourier transform, principal component analysis, factor analysis, clustering, etc... ^ [The fourier transform case is insteresting because it describes an *ordered* sample of size $N$ with two ordered seires of $N/2$ numbers (one real and one imaginary) which sum up to $2N$ numbers (the two series plus their respective order). This is much less than the assumed $N^1-1$ needed in general. I suspect that this is because for this to happen, a regularly sampled series is needed. With this restriction, the fourier transform can "compress" the information.]. The issue is not plots vs. numbers but "all the numbers" vs. "just some numbers". The big advantage of plots is that they can show an enormous amount of numbers efficiently and intuitivelly, in adition allowing to see a *gestalt* that is impossible to get by just looking at series of numbers. 
...
Boxplot try to represent a sample with about 5 numbers. Hence, it is expeceted to have metamerism for samples with $N>5$. A density estimation using parametric methods, on the other hand, can be evaluated at potentially infinite points even for small samples. The possibility of metamerism in this case depends on the "resolution" with wich the curve is described. If it is rendered with fewer points than the sample size, then it will metamerise. 
...
This motivates to define the category of "effective" statistical transormations as transformation that can uniquely describe a univariate sample of size N with, at most, N numbers. Under this definition, "the first N moments" is effective, while "the first moment repeated N times" is no. At this point, this is pure speculation, so take it with a grain of salt. 
...
Moreover, `metamerize()` can be piped, saving the parameters of each call (excepto `N` and `trim`). This way one can perform sequences.
...
This metamers show the datasaurus metamorphosing into differnet figures, always preserving the same statistical properties. This replicates [Justin Matejka's y George Fitzmaurice's animation](https://www.autodeskresearch.com/publications/samestats)
