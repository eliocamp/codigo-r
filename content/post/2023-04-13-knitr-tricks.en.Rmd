...
So here are the 12 tips I decided to include in no particular order.
...
This "[hook](https://yihui.org/knitr/hooks/)" runs once for each chunk in which `label` is not `NULL` --which is all of them-- and throws an error if the `fig.cap` option is `NULL` (missing).
