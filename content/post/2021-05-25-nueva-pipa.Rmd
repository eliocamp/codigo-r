...
Ese `%>%` es el operador que permite encadenar una función tras otra sin neceisdad de asignar variables a pasos intermedios.
...
(No que la cantidad de caracteres importe mucho si uno usa el atajo de RStudio Ctrl + Shitf + M. Y con la nueva versión de RStudio [que ahora está en preview](https://web.archive.org/web/20210525152348/https://www.rstudio.com/products/rstudio/download/preview/), se puede elegir cuál usar.)
...
Con este cambio, ya no necesito empezar cada script con `library(magrittr)` (aunque ver [la siguiente seccion]).

Pará quienes tienen una (¿insana?) obsesión con la velocidad y la eficiencia, `|>` [parecer ser más rápida](https://michaelbarrowman.co.uk/post/the-new-base-pipe/) que `%>%`.
...
no es literalmente equivalente a `mean(x)`; hay todo un inverso de procesamiento adentro de esos tres caracteres.
Mientas que
...
Como el primer argumento de `lm()` no es los datos, hay que usar `data = .` para decirle a magrittr que `datos` no tiene que ser el primer argumento de `lm()`.
...
Desde R 4.1.0 estas dos expresiones son equivalentes:
...
El truco es que R se fija en el nombre de la función, así que lo único que hay que hacer es renombrar la función `[` (amo que en R todo sea una función).
...
R 4.1.0 salío hace un par de semanas y es muy probable que la mayor parte de la gente no hay actualizado ni tenga planes de actualizar pronto.
...
Por todo esto, si bien el reino de magrittr ya tiene los días contados, todavía está lejos de terminar.
